...
def greedy_path_builder(final_pois_df, distance_matrix, idx2poiid, iteration_POI_graph, request_params):
    ...
    Args:
        ...
        request_params: Dictionary with keys ['start_node', 'end_node', 'time_constraint']
    ...
    end_node = request_params['end_node'] if request_params.get('end_node') is not None else start_node
    ...
    while True:
        ...
        neighbors = [n for n in iteration_POI_graph.neighbors(current_node) if n not in visited and n != end_node]
        if not neighbors:
            break
        ...
        dist_back_to_start = distance_matrix[nearest_neighbor][end_node]
        ...
        temporal_dist_back_to_start = distance_matrix[nearest_neighbor][end_node]/walking_speed
...
if __name__ == '__main__':
    #%%
    CITY = 'tokyo' #'bonn' # Specify city name here: 'berlin', 'bonn', 'hamburg', 'tokyo', 'new york', 'verona'
    end_node_variant = False # False for other datasets
    POI_graph_threshold = 3000#3000 # in meters
    EXP_NAME = f'baseline_greedy_distance_{CITY}_th{POI_graph_threshold}_duplicate'
    ...
    results_df = pd.DataFrame(
        columns = ['req_id', 'start_node', 'end_node', 'time_constraint', 'route', 'inference_time']
    )
    if end_node_variant:
        results_df = pd.DataFrame(
            columns = ['req_id', 'start_node', 'end_node', 'time_constraint', 'route', 'inference_time']
    ...
    for idx, row in tqdm(test_set.iterrows(), total = len(test_set)):
        ...
        if end_node_variant:
            end_node = poiid2idx[row['end_node']]
        ...
        route = greedy_path_builder(
            ...
            iteration_POI_graph,
            {
                ...
                'end_node': end_node if end_node_variant else None,
        ...
        results_df.loc[idx] = [
            req_id,
            start_node,
            end_node if end_node_variant else None,
            time_constraint,
            route,
            inference_time
        ]
